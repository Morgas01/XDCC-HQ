...
let searches={
	...
	"FILE":function(query)
	{
		let parsedFile=new SC.File(niwaWorkDir).changePath("work/"+appContext).changePath(subOfficeName+"on");
	...
	"BOT":function(query)
	{
		let downloadedFile=new SC.File(niwaWorkDir).changePath("storage").changePath(subOfficeName.slice(0,-3)+".txt");
		let parsedFile=new SC.File(niwaWorkDir).changePath("storage").changePath(subOfficeName.slice(0,-3)+".json");
		return parsedFile.exists()
		.then(function()
		{
			//age in days < file expiration time
