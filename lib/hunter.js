...
var searches={
	...
	"BOT":function(query)
	{
		var downloadedFile=new SC.File(__dirname).changePath("../storage").changePath(subOfficeName.slice(0,-3)+".txt");
		var parsedFile=new SC.File(__dirname).changePath("../storage").changePath(subOfficeName.slice(0,-3)+".json");
		return parsedFile.exists()
		.then(function()
		{
			//age in days < file expiration time
