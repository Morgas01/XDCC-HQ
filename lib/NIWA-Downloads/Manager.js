(function(µ,SMOD,GMOD,HMOD,SC){
    ...
    var delegateDownload=function(appName,download,onUpdate,manager)
    {
    	...
    	var p=worker.ask(appName,"receiveDownload",{ID:delegateInfo.ID,download:download});
    ...
    worker.updateDelegatedDownload=function(update,appName)
	{
		...
		else
		{
			...
			if(runningDownload.state!==SC.Download.states.RUNNING) delegateMap.delete(update.ID);
		}
	}
	...
	var MANAGER=module.exports=µ.Class({
    	...
    	receiveDownload:function(data,appName)
    	{
			...
			var downloadClass=this.DBClassDictionary[data.download.objectType];
    	...
    	_trigger:function()
    	{
    		...
    		if(this.maxDownloads!=0&&this.runningDownloadMap.size>=this.maxDownloads) return;
    		//TODO queue?
    		...
    		triggerPromise.catch(µ.logger.error);
