(function(µ,SMOD,GMOD,HMOD,SC){
    ...
    module.exports=function(options)
    {
    		...
    		return dbConnector.then(dbc=>
    		{
				...
				.map(type=>
				{
					...
					if(dbClass.prototype instanceof SC.Download||dbClass==SC.Download) //filter downloads
						...
						loading=loading.then(packages=>service.fetchPackages(packages)
						.then(()=>packages.map(p=>
						{
							...
							SC.NodePatch.traverse(p,function(package)
							{
								...
								{
									var parent=package;
									while(parent=package.getParent("package")) toDelete.delete(parent);
								}
							});
				...
				.then(deletion=>Array.prototype.concat.apply(Array.prototype,deletion))//flatten
				.then(items=>items.reduce((dict,item)=>
				{
					var type=item.objectType;
					if(type in dict) dict[type].push(item);
					else dict[type]=[item];
					return dict;
				},{}))
				.then(dict=>
    	...
    	service.fetchPackages=function(packages)
    	{
    		...
    		return dbConnector.then(dbc=>
    			Promise.all(packages.map(p=>
    				Promise.all([
    					...
    					dbc.loadChildren(p,"subPackages").then(service.fetchPackages)
					])
				))
			)
    	}
